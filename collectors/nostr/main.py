...
NOSTR_RELAYS = os.getenv("NOSTR_RELAYS", ",".join([
    "wss://purplepag.es",
    "wss://nostr.mom",
        ...
        try:
            async with websockets.connect(
                ...
                async for message in ws:
                    ...
                    if count % 50 == 0:
                        producer.flush()
                        logger.info("Progression | relay=%s events_produits=%d topic=%s",
                                    relay_url, count, TOPIC)
        except websockets.exceptions.ConnectionClosed as e:
            logger.warning("Relay déconnecté | relay=%s code=%s | nouvelle tentative dans %ds",
                           relay_url, e.code, RECONNECT_DELAY)
            await asyncio.sleep(RECONNECT_DELAY)
        except Exception as e:
            logger.error("Erreur relay | relay=%s type=%s message=%s | nouvelle tentative dans %ds",
                         relay_url, type(e).__name__, e, RECONNECT_DELAY)
            await asyncio.sleep(RECONNECT_DELAY)
