...
def collect():
    ...
    logger.info(
        "Intervalle initial calculé | interval=%ds requetes_par_poll=%d",
        int(poll_interval),
        REQUESTS_PER_POLL,
    )
    ...
    while True:
        if quota_remaining <= QUOTA_RESERVE:
            ...
            logger.warning(
                "Quota épuisé | quota_restant=%d pause_jusqu_au_reset=%.0fs (%.1fh)",
                quota_remaining,
                wait,
                wait / 3600,
            )
        ...
        for site in SITES:
            try:
                ...
                logger.debug(
                    "Site traité | site=%s questions=%d réponses=%d quota_restant=%d",
                    site,
                    len(questions),
                    len(answers),
                    quota_remaining,
                ...
                logger.error(
                    "Erreur collecte | site=%s type=%s message=%s",
                    site,
                    type(e).__name__,
                    e,
                )
